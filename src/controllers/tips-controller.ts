...
export class TipController {
  ...
  public static async getSingleTip  (req: Request, res: Response)  {
    try {
      const { id } = req.params
  ...
  public static async updateTip  (
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Dica atualizada com sucesso", updatedTip })
    } catch (error) {
      console.log(error)
      return res.status(400).json({ error })
    }
  ...
  public static async deleteTip  (req: Request, res: Response)  {
    try {
      const { id } = req.params
      ...
      if (!Types.ObjectId.isValid(id)) {
        return res
          .status(500)
          .json({ message: "Erro ao eliminar. Id inválido" })
      }
